class BaseMQ(object):
    ...
    def _init_(self, pinData, pinHeater=-1, boardResistance = 10, baseVoltage = 3.3, measuringStrategy = STRATEGY_ACCURATE):
    ...
    def calibrate(self, ro=-1):
        if ro == -1:
            ...
            for i in range(0,MQ_SAMPLE_TIMES + 1):        
                ...
                ro += self._calculateResistance_(self.pinData.read_u16())
    ...
    def _calculateResistance_(self, rawAdc):
    ...
    def _readRs_(self):
        if self.measuringStrategy == STRATEGY_ACCURATE :            
                rs = 0
                for i in range(0, MQ_SAMPLE_TIMES + 1): 
                    rs += self._calculateResistance_(self.pinData.read_u16())
                    utime.sleep_ms(MQ_SAMPLE_INTERVAL)
